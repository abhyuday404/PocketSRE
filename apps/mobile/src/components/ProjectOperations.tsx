...
export function ProjectOperations({
  ...
  return (
    <View style={{ gap: 14 }}>
      ...
      <Text style={ui.body}>
        Connect Vercel, confirm the matching project and environment, then refresh project health to
        collect build and runtime log evidence. Access and log retention depend on the provider.
      </Text>
      {!provider?.connected ? (
        <>
          <TextInput
          ...
          />
          <Text style={ui.label}>
            Use a token scoped to the intended project/team. It stays in gateway memory and must be
            reconnected after a restart unless configured by its administrator.
          </Text>
          <Button
      ...
      <Text style={ui.body}>
        The gateway checks enabled projects while the phone is closed. Keep the gateway running. Two
        consecutive failures trigger an outage; two healthy checks trigger recovery. Unknown results
        do not establish recovery.
      </Text>
      <Badge>{project.monitoring ? 'Background monitoring enabled' : 'Monitoring paused'}</Badge>
      <Text style={ui.label}>
