...
- Offline bundles and validated diagnoses use private app document storage, not application-level encryption. The cache retains five snapshots per incident and up to thirty per gateway, import, or sample collection, with a previous generation for interrupted-write recovery. Clearing saved data removes both generations across all gateways. Exported files are outside the cache lifecycle; delete those separately. See [offline history](../apps/mobile/src/storage/README.md).
